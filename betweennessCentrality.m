function betweenfn = node_betweenfneenness_slow(adj)
n = numnodes(adj);
spaths=inf(n,n);
for k=1:n-1
  adjk=adj^k;
...
for i=1:n
    ...
    for j=1:n
        ...
        betweenfn(route_ji(2:length(route_ji)-1)) = betweenfn(route_ji(2:length(route_ji)-1)) + 1/spaths(j,i);
    end
end
betweenfn=betweenfn/nchoosek(n,2);
...
n = size(L,2);
L(find(L==0))=Inf;  % zero distances made equal to infinity
for i=1:n
  J(steps,i) = L(i,t); 
